...
public class PageRanksView extends AbstractXlsView{
	...
	protected void buildExcelDocument(Map<String, Object> model, Workbook workbook, HttpServletRequest request,
		...
		//행번호 지정
		int rowNum=1; //0은 항목(레이블)을 명시하기 위해 이미 사용
	...
	private void createColumnLabel(HSSFSheet sheet) {
		...
		HSSFCell cell = firstRow.createCell(0);//첫번째 행(가로) 생성
	...
	private void createPageRankRow(HSSFSheet sheet, PageRank rank, int rowNum) {
		HSSFRow row = sheet.createRow(rowNum);//rowNum 증가시키면서 행 개수 증가
