...
<div class = "page-main">
	...
	<form action = "admin_list.do" id = "search_form" method = "get">
		<ul class = "search">
			...
			<li>                                                         <!-- ${param.keyword} 정보가 남아 있음.. 왜 그런지는 나중에 따로 찾아봐야함 -->
	...
	<table class = "striped-table">
		...
		<c:forEach var = "member" items = "${list}">
			<tr>
				...
				<td>
					<c:if test="${member.auto==0}">탈퇴</c:if>
					<c:if test="${member.auto==1}">정지</c:if>
					<c:if test="${member.auto==2}">일반</c:if>
					<c:if test="${member.auto==9}">관리</c:if>
				</td>				
			</tr>
		</c:forEach>
	</table>
