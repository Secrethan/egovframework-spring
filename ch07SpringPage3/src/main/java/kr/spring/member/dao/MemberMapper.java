...
public interface MemberMapper {
	...
	@Update("UPDATE spmember SET nick_name=#{nick_name} "
			+ "WHERE mem_num=#{mem_num}")
	public void updateMember(MemberVO member);
	@Update("UPDATE spmember_detail SET name=#{name},"
		  + "phone=#{phone},email=#{email},"
		  + "zipcode=#{zipcode},address1=#{address1},"
		  + "address2=#{address2},hobby=#{hobby},"
		  + "modify_date=SYSDATE WHERE mem_num=#{mem_num}")
	public void updateMember_detail(MemberVO member);
	@Update("UPDATE spmember_detail SET "
	...
	//프로필 이미지 업데이트
	@Update("UPDATE spmember_detail SET photo=#{photo},"
	...
	@Select("SELECT m.mem_num,m.id,m.auth,m.au_id,"
		  + "d.passwd,m.nick_name,d.email FROM "
		  + "spmember m JOIN spmember_detail d ON "
		  + "m.mem_num=d.mem_num WHERE m.au_id=#{au_id}")
